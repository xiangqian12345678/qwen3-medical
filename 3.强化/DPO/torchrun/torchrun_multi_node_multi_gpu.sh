...
# DPO分布式训练脚本 - 基于torchrun实现多节点多GPU训练
# 
# 使用方法:
# 1. 单节点多GPU训练:
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  $0 --nproc_per_node 4 --use_deepspeed true"
            echo ""
            echo "  # 使用自定义DeepSpeed配置"
            echo "  $0 --nproc_per_node 4 --use_deepspeed true --deepspeed_config zero3.json"
            exit 0
            ;;
        *)
            echo "未知参数: $1"
...
OUTPUT_DIR="$PROJECT_ROOT/output/dpo_adapter"
...
TRAIN_CMD="$TRAIN_CMD dpo_training.py \
